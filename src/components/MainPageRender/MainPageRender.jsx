...
import { companies, findGrowth, marketCap } from "../../constants";
...
export default function MainPageRender({pageIndex, searchResult}) {
    ...
    //         i++;
    //     });
    //     return <div className="MainPageRender"><Stock companyName={output.name} valuation={marketCap(output.investments[output.investments.length-1].amount, output.investments[output.investments.length-1].equity)} growthPercent={result[1] ? "+"+result[0] : "-"+result[0]} color={result[1] ? "#41C3A9" : "#FF7972"} lineData={lineData} idea={output.idea} mark={true} /></div>;
    // }
    if (pageIndex === 0) {
        return <MainPage />;
    } else if (pageIndex === 1) {
